...
const getListings = async () => {
    try {
        const res = await fetch('https://lost-found-gray.vercel.app/api/listing', {
            cache: 'no-store'
        })
        if (res.ok) {
    ...
    return (
        <>
            {listings && listings.length > 0 ? (listings.map((l) => {
                ...
                return (
                    <div className='border border-black h-96 w-1/4 flex flex-col rounded-lg mt-12 p-2 gap-2 mb-4 overflow-hidden text-ellipsis whitespace-nowrap' key={l.id}>
